...
resource "aws_route53_record" "for-prod-cloudfront-validation" {
  ...
  records = [
    tolist(aws_acm_certificate.for-prod-cloudfront.domain_validation_options)[0].resource_record_value]
  ttl = "300"
...
resource "aws_route53_record" "for-prod-load-balancer-validation" {
  zone_id = data.aws_route53_zone.public.zone_id
  ...
  records = [
    tolist(aws_acm_certificate.for-prod-load-balancer.domain_validation_options)[0].resource_record_value]
  ttl = "300"
  allow_overwrite = true
}